...
const navItems: NavItem[] = [
  ...
  { href: '/inbox', labelKey: 'inbox', icon: ChatCircleDotsIcon },
...
export function Sidebar({ open = false, onClose }: SidebarProps) {
  ...
  const { isPlatformAdmin, loading: platformAdminLoading } =
    useIsPlatformAdmin();
  const pendingAccessRequests = usePendingAccessRequests();
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          ...
          {!platformAdminLoading && isPlatformAdmin ? (
            <>
              <div className="border-border my-4 border-t" />

              <ul className="flex flex-col gap-1">
                <li key="/platform/accounts">
                  <Link
                    href="/platform/accounts"
                    className={cn(
                      'flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors lg:py-2',
                      pathname.startsWith('/platform/accounts')
                        ? 'bg-primary/10 text-primary'
                        : 'text-muted-foreground hover:bg-muted hover:text-foreground'
                    )}
                  >
                    <Shield className="h-4 w-4" />
                    <span className="flex-1">{t('platformAdmin')}</span>
                  </Link>
                </li>
                <li key="/platform/customer-invites">
                  <Link
                    href="/platform/customer-invites"
                    className={cn(
                      'flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors lg:py-2',
                      pathname.startsWith('/platform/customer-invites')
                        ? 'bg-primary/10 text-primary'
                        : 'text-muted-foreground hover:bg-muted hover:text-foreground'
                    )}
                  >
                    ...
                    <span className="flex-1">
                      {t('platformCustomerInvites')}
                    </span>
                  </Link>
                </li>
                <li key="/platform/access-requests">
                  <Link
                    href="/platform/access-requests"
                    className={cn(
                      'flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors lg:py-2',
                      pathname.startsWith('/platform/access-requests')
                        ? 'bg-primary/10 text-primary'
                        : 'text-muted-foreground hover:bg-muted hover:text-foreground'
                    )}
                  >
                    ...
                    <span className="flex-1">
                      {t('platformAccessRequests')}
                    </span>
                    {pendingAccessRequests > 0 && (
                      <span className="bg-primary text-primary-foreground flex h-5 min-w-5 items-center justify-center rounded-full px-1 text-[10px] font-semibold">
                        {pendingAccessRequests > 9
                          ? '9+'
                ...
                <li key="/platform/settings">
                  ...
                  >
                    ...
                    <span className="flex-1">{t('platformSettings')}</span>
                  </Link>
                </li>
              </ul>
            </>
