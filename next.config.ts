...
const SECURITY_HEADERS = [
  ...
  },
  {
    ...
    value: [
      ...
      // connect.facebook.net hosts the Facebook JS SDK, injected on
      // demand for Meta Embedded Signup (Settings → WhatsApp).
      ...
      "font-src 'self' data:",
      // Supabase REST + realtime (WSS). graph.facebook.com is only in
      // this list because the Facebook JS SDK's FB.login performs a
      // handshake from the page context; the actual Embedded Signup
      // flow runs inside Meta's own popup. All WhatsApp API traffic
      // stays server-side.
